...
import { moveUPSelectColumn } from "@/lib/Redux/Slices/commonSlices/SelectedItemSlice";

export default function UpNavButton() {
  const rightSelectedItems = useAppSelector(selectRightSelectedItem);
  const dispatch = useAppDispatch();
  function btn_click() {
    if (rightSelectedItems.length !== 0) {
      dispatch(moveUPSelectColumn(rightSelectedItems[0]));
    }
  }
